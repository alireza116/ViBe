...
│   │   ├── point.js · face.js · bar.js · rect.js · tick.js · text.js · line.js · rule.js · composite.js
...
**Intentional exception — `face`:** the eyes/brows/mouth *are* the controls (`dm` tracks). `handles` gates only the supplemental eyelid/lip/size dots.
...
**Parametric glyphs** (`trend`, `trendBand`, `face`) and **map chrome** (`geoBasemap`, `geoTile`) are not create targets — document create-fitness per mark (see `MARK_CONTRACTS.md`).
