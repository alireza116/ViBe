...
function resolveChannel(name, spec, data, dims) {
    ...
    return {
        ...
        invertible: scale.invertible,
        encode: (datum) => scale.encode(datum[field]),
        invert: (visual) => scale.invertValue(visual),
        domain: () => domain
    };
}
...
export function datumFromPointer(enc, { x, y }) {
    const datum = {};
    for (const [name, px] of [['x', x], ['y', y]]) {
        const ch = enc[name];
        if (ch && ch.invertible && ch.field !== undefined && px !== undefined) {
